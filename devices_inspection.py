...
def inspection(device_info, cmds_dict):
    ...
    try:  # 尝试登录设备
        ...
        log = open(os.getcwd() + '\\' + localtime + '\\' + 'log.log', 'a')  # 创建log文件保存巡检报错的设备故障信息
        log.write('设备 ' + device_info['host'] + ' 登录信息错误！\n')
        log.close()
    except (NetmikoTimeoutException):  # 登录信息中IP地址不可达
        print(f'设备 {device_info["host"]} 管理地址不可达！')
        log = open(os.getcwd() + '\\' + localtime + '\\' + 'log.log', 'a')
        log.write('设备 ' + device_info['host'] + ' 管理地址不可达！\n')
        log.close()
...
if __name__ == '__main__':
    ...
    if not os.path.exists(localtime):  # 检查是否有同日期命名的相同文件夹
        ...
        try:  # 尝试删除巡检故障设备报错的log文件
            os.remove(os.getcwd() + '\\' + localtime + '\\' + 'log.log')  # 删除log文件
